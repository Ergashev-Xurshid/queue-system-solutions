...
function LoginPage() {
  ...
  useEffect(() => {
    const checkToken = async () => {
      ...
      const role = localStorage.getItem("role"); // login paytida saqlangan

      if (!token || role !== "ROLE_ADMIN") return; // admin emas
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch("https://navbat.kstu.uz/auth/login", {
        method: "POST",
      ...
      // Token va role alohida saqlaymiz
      localStorage.setItem("token", data.body.token);
      localStorage.setItem("role", data.body.role);
      setToken(data.body.token); // token saqlash
      navigate("/admin", { replace: true }); // yo‘naltirish
    } catch (err) {
      console.error(err);
